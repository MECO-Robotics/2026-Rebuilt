...
public class Hopper {
  ...
  public static final String GAMEPIECE_POSES_LOG_KEY =
      "Visualization/RobotRemy/Hopper/GamePiecePoses";
  ...
  public Hopper(
      IntSupplier ballCountSupplier,
      DoubleSupplier intakeExtensionSupplier,
  ...
  /** Returns field-relative poses for all game pieces currently held inside the robot hopper. */
  ...
  private List<Translation3d> buildLocalBallCenters() {
    ...
    // Triangular layer spacing for sphere stacking (instead of axis-aligned cube packing).
    double verticalPitch = Math.sqrt(3.0) / 2.0 * diameter;
    double intakeFrontProjection =
        Math.max(0.0, intakeExtensionSupplier.getAsDouble())
    ...
    for (double z = bottomLimit + radius; z <= topLimit - radius + 1e-9; z += verticalPitch) {
      ...
      int xIndex = 0;
      for (double x = backLimit + radius + xOffset;
          x <= frontLimit - radius + 1e-9;
          x += diameter) {
        // Alternate lateral offset for neighboring stacks to mimic sphere nesting in 3D.
        double yOffset = ((zIndex + xIndex) % 2 == 0) ? 0.0 : radius;
        for (double y = rightLimit + radius + yOffset;
            y <= leftLimit - radius + 1e-9;
            y += diameter) {
