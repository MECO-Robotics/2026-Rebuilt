 ...
 */
public class TunableArmFeedforward implements PositionJointFeedforward, ProtobufSerializable, StructSerializable {
	...
	public TunableArmFeedforward(double ks, double kg, double kv, double ka, double dtSeconds) {
		...
		if (dtSeconds <= 0.0) {
			throw new IllegalArgumentException("period must be a positive number, got " + dtSeconds + "!");
		}
		m_dt = dtSeconds;
	}

	/**
	 * Creates a new ArmFeedforward with the specified gains. The period is
	 * defaulted to 20 ms.
	 *
	 * @param ks
	 *            The static gain in volts.
	 ...
	 *            The position (angle) setpoint. This angle should be measured from
	 *            the horizontal (i.e. if the provided angle is 0, the arm should be
	 ...
	 * continuous control (acceleration is assumed to be zero).
	 *
	 * @param positionRadians
	 *            The position (angle) setpoint. This angle should be measured from
	 *            the horizontal (i.e. if the provided angle is 0, the arm should be
	 ...
	 *
	 * @param currentAngle
	 *            The current angle in radians. This angle should be measured from
	 *            the horizontal (i.e. if the provided angle is 0, the arm should be
	 *            parallel to the floor). If your encoder does not follow this
	 *            convention, an offset should be added.
	 ...
	 */
	public double calculate(double currentAngle, double currentVelocity, double nextVelocity, double dt) {
		return ArmFeedforwardJNI.calculate(ks, kv, ka, kg, currentAngle, currentVelocity, nextVelocity, dt);
	}

	/**
	 * Calculates the feedforward from the gains and setpoints assuming discrete
	 * control when the velocity does not change.
	 *
	 * @param currentAngle
	 *            The current angle. This angle should be measured from the
	...
	public Voltage calculate(Angle currentAngle, AngularVelocity currentVelocity) {
		return Volts.of(kg * Math.cos(currentAngle.in(Radians)) + ks * Math.signum(currentVelocity.in(RadiansPerSecond))
	...
	public Voltage calculate(Angle currentAngle, AngularVelocity currentVelocity, AngularVelocity nextVelocity) {
		return Volts.of(ArmFeedforwardJNI.calculate(ks, kv, ka, kg, currentAngle.in(Radians),
				currentVelocity.in(RadiansPerSecond), nextVelocity.in(RadiansPerSecond), m_dt));
	}
	 ...
	 * position, and an acceleration. Useful for ensuring that velocity and
	 * acceleration constraints for a trapezoidal profile are simultaneously
	 * achievable - enter the acceleration constraint, and this will give you a
	 * simultaneously-achievable velocity constraint.
	 *
	 * @param maxVoltage
	 *            The maximum voltage that can be supplied to the arm.
	 * @param angle
	 *            The angle of the arm, in radians. This angle should be measured
	 *            from the horizontal (i.e. if the provided angle is 0, the arm
