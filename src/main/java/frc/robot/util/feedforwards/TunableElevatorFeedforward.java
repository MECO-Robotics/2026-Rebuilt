 ...
 */
public class TunableElevatorFeedforward implements PositionJointFeedforward, ProtobufSerializable, StructSerializable {
	...
	/**
	 * Creates a new ElevatorFeedforward with the specified gains. Acceleration gain
	 * is defaulted to zero. The period is defaulted to 20 ms.
	 *
	 * @param ks
	 *            The static gain in volts.
	 * @param kg
	...
	public TunableElevatorFeedforward() {
		this(0, 0, 0, 0);
	}
	 ...
	 * control when the setpoint does not change.
	 *
	...
	public Voltage calculate(LinearVelocity currentVelocity, LinearVelocity nextVelocity) {
		...
		if (ka == 0.0) {
			return Volts.of(
					ks * Math.signum(nextVelocity.in(MetersPerSecond)) + kg + kv * nextVelocity.in(MetersPerSecond));
		} else {
			double A = -kv / ka;
			double B = 1.0 / ka;
			double A_d = Math.exp(A * m_dt);
			double B_d = 1.0 / A * (A_d - 1.0) * B;
			return Volts.of(kg + ks * Math.signum(currentVelocity.magnitude())
					+ 1.0 / B_d * (nextVelocity.in(MetersPerSecond) - A_d * currentVelocity.in(MetersPerSecond)));
		}
	}
	...
	public double calculate(double position, double currentVelocity, double nextVelocity, double dt) {
		...
		if (ka == 0.0) {
			...
			double A_d = Math.exp(A * m_dt);
			double B_d = 1.0 / A * (A_d - 1.0) * B;
			return kg + ks * Math.signum(currentVelocity) + 1.0 / B_d * (nextVelocity - A_d * currentVelocity);
		}
	}
	 ...
	 * an acceleration. Useful for ensuring that velocity and acceleration
	 * constraints for a trapezoidal profile are simultaneously achievable - enter
	 * the acceleration constraint, and this will give you a
	 ...
	 * and a velocity. Useful for ensuring that velocity and acceleration
	 * constraints for a trapezoidal profile are simultaneously achievable - enter
	 * the velocity constraint, and this will give you a simultaneously-achievable
