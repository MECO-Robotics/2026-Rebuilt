...
public class AdvancedPPHolonomicDriveController implements PathFollowingController {
	 ...
	 *            PID constants for the translation PID controllers
	 * @param rotationConstants
	 *            PID constants for the rotation controller
	 * @param period
	 *            Period of the control loop in seconds
	 * @param translationalP
	 *            Tunable translation P gain
	 * @param translationalI
	 *            Tunable translation I gain
	 * @param translationalD
	 *            Tunable translation D gain
	 * @param rotationalP
	 *            Tunable rotation P gain
	 * @param rotationalI
	 *            Tunable rotation I gain
	 * @param rotationalD
	 *            Tunable rotation D gain
	 */
	public AdvancedPPHolonomicDriveController(PIDConstants translationConstants, PIDConstants rotationConstants,
			double period, LoggedTunableNumber translationalP, LoggedTunableNumber translationalI,
			LoggedTunableNumber translationalD, LoggedTunableNumber rotationalP, LoggedTunableNumber rotationalI,
		...
		this.xController = new PIDController(translationalP.get(), translationalI.get(), translationalD.get(), period);
		this.xController.setIntegratorRange(-translationConstants.iZone, translationConstants.iZone);

		this.yController = new PIDController(translationalP.get(), translationalI.get(), translationalD.get(), period);
		...
		this.rotationController = new PIDController(rotationalP.get(), rotationalI.get(), rotationalD.get(), period);
	...
	public AdvancedPPHolonomicDriveController(PIDConstants translationConstants, PIDConstants rotationConstants,
			double period) {
		this(translationConstants, rotationConstants, period,
	 ...
	 * called on a disabled controller, only feedforward values are returned.
	 *
	 * @param enabled
	...
	public ChassisSpeeds calculateRobotRelativeSpeeds(Pose2d currentPose, PathPlannerTrajectoryState targetState) {
		...
		double xFeedback = this.xController.calculate(currentPose.getX(),
				targetState.pose.getX() + xSetpointIncrement.getAsDouble());
		double yFeedback = this.yController.calculate(currentPose.getY(),
				targetState.pose.getY() + ySetpointIncrement.getAsDouble());
		...
		}, translationalP, translationalI, translationalD, rotationalP, rotationalI, rotationalD);
	 ...
	 * in functionality in the path following command.
	 *
	 ...
	 * the path
	 *
	 * @param rotationTargetOverride
	 *            Supplier to override rotation targets
	 * @deprecated Use overrideRotationFeedback instead, with the output of your own
	 *             PID controller
	 */
	@Deprecated
	 ...
	 *
	 * @param xFeedbackOverride
	 *            Double supplier that returns the desired FIELD-RELATIVE X feedback
	 *            in meters/sec
	 */
	...
	/**
	 * Stop overriding the X axis feedback, and return to calculating it based on
	 ...
	 *
	 * @param yFeedbackOverride
	 *            Double supplier that returns the desired FIELD-RELATIVE Y feedback
	 *            in meters/sec
	 */
	...
	/**
	 * Stop overriding the Y axis feedback, and return to calculating it based on
	 * path following error.
	 */
	...
	/**
	 * Stop overriding the X and Y axis feedback, and return to calculating them
	 * based on path following error.
	 */
	 ...
	 *
	 * @param rotationFeedbackOverride
	 *            Double supplier that returns the desired rotation feedback in
	 *            radians/sec
	 */
	...
	/**
	 * Stop overriding the rotation feedback, and return to calculating it based on
	 * path following error.
	 */
	...
	/**
	 * Clear all feedback overrides and return to purely using path following error
	 * for feedback
	 */
