...
import static frc.robot.util.mechanical_advantage.EqualsUtil.*;
...
import frc.robot.util.mechanical_advantage.EqualsUtil;
...
public class SwerveSetpointGenerator {
	 ...
	 *
	 * @param prevToGoal
	 *            The rotation from the previous state to the goal state (i.e.
	 ...
	 *
	 * @param func
	 *            The Function2d to take the root of.
	 * @param x_0
	 *            x value of the lower bracket.
	 * @param y_0
	 *            y value of the lower bracket.
	 * @param f_0
	 *            value of 'func' at x_0, y_0 (passed in by caller to save a call to
	 *            'func' during recursion)
	 * @param x_1
	 *            x value of the upper bracket.
	 * @param y_1
	 *            y value of the upper bracket.
	 * @param f_1
	 *            value of 'func' at x_1, y_1 (passed in by caller to save a call to
	 *            'func' during recursion)
	 * @param iterations_left
	 *            Number of iterations of root finding left.
	 ...
	 *
	 * @param limits
	 *            The kinematic limits to respect for this setpoint.
	 * @param prevSetpoint
	 *            The previous setpoint motion. Normally, you'd pass in the previous
	 *            iteration setpoint instead of the actual measured/estimated
	 *            kinematic state.
	 * @param desiredState
	 *            The desired state of motion, such as from the driver sticks or a
	 ...
	 */
	public SwerveSetpoint generateSetpoint(final ModuleLimits limits, final SwerveSetpoint prevSetpoint,
			ChassisSpeeds desiredState, Translation2d centerOfRotation, double dt) {
		...
		if (all_modules_should_flip
				&& !EqualsUtil.GeomExtensions.epsilonEquals(GeomUtil.toTwist2d(prevSetpoint.chassisSpeeds()),
						new Twist2d())
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			if (epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
				...
				var necessaryRotation = prevSetpoint.moduleStates()[i].angle.unaryMinus()
			...
			double s = findSteeringMaxS(prev_vx[i], prev_vy[i], prev_heading[i].getRadians(), desired_vx[i],
					desired_vy[i], desired_heading[i].getRadians(), max_theta_step, kMaxIterations);
			min_s = Math.min(min_s, s);
		}

		Logger.recordOutput("Drive/ModulesDesiredVx", desired_vx);
		Logger.recordOutput("Drive/ModulesDesiredVy", desired_vy);

		Logger.recordOutput("Drive/ModulesPrevVx", prev_vx);
		Logger.recordOutput("Drive/ModulesPrevVy", prev_vy);

		Logger.recordOutput("Drive/ModuleAccelerating",
				Math.hypot(desired_vx[0], desired_vy[0]) - Math.hypot(prev_vx[0], prev_vy[0]));
		// Enforce drive wheel acceleration limits.
		double max_vel_step;

		// Assume all modules are decelerating
		max_vel_step = dt * limits.maxDriveDeceleration();

		for (int i = 0; i < modules.length; i++) {
			if (Math.hypot(desired_vx[i], desired_vy[i]) > Math.hypot(prev_vx[i], prev_vy[i])) {
				// One module is accelerating, enforce acceleration limits, if one module is
				// accelerating,
				// the rest must follow along
				max_vel_step = dt * limits.maxDriveAcceleration();
				break;
			}
		}

		Logger.recordOutput("Drive/ModulesMaxVelStep", max_vel_step);

		for (int i = 0; i < modules.length; ++i) {
			// double max_vel_step;
			// if (Math.hypot(desired_vx[i], desired_vy[i]) > Math.hypot(prev_vx[i],
			// prev_vy[i])) {
			// // Module is accelerating, enforce acceleration limits
			// max_vel_step = dt * limits.maxDriveAcceleration();
			// } else {
			...
			double s = min_s * findDriveMaxS(prev_vx[i], prev_vy[i], Math.hypot(prev_vx[i], prev_vy[i]), vx_min_s,
					vy_min_s, Math.hypot(vx_min_s, vy_min_s), max_vel_step, kMaxIterations);
			min_s = Math.min(min_s, s);
		}

		ChassisSpeeds retSpeeds = new ChassisSpeeds(prevSetpoint.chassisSpeeds().vxMetersPerSecond + min_s * dx,
		...
		double[] azimuthVelocityFF = new double[4];
		...
		for (int i = 0; i < modules.length; ++i) {
			...
			azimuthVelocityFF[i] = prevSetpoint.moduleStates()[i].angle.unaryMinus().rotateBy(retStates[i].angle)
					.getRotations() / 0.02;
		}
		return new SwerveSetpoint(retSpeeds, retStates, azimuthVelocityFF);
	}
}
