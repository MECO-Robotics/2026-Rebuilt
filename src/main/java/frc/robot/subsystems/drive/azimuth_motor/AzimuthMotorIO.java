...
public interface AzimuthMotorIO {
	...
	public static Supplier<AzimuthMotorIO> talonFXFactory(String name, AzimuthMotorHardwareConfig config) {
		return () -> new AzimuthMotorIOTalonFX(name, config);
	}

	/** Creates a SparkMax-backed azimuth motor IO supplier. */
	public static Supplier<AzimuthMotorIO> sparkMaxFactory(String name, AzimuthMotorHardwareConfig config) {
		return () -> new AzimuthMotorIOSparkMax(name, config);
	}

	/** Creates a default simulation azimuth motor IO supplier. */
	public static Supplier<AzimuthMotorIO> simFactory(String name, AzimuthMotorHardwareConfig config) {
	 ...
	 */
	public static AzimuthMotorIO fromMode(String name, AzimuthMotorHardwareConfig config,
	 ...
	 *
	 * <p>
	 * Allows callers to provide a custom sim factory (for example, MapleSim-backed
	 * module IO endpoints that share state with drive IO).
	 */
	public static AzimuthMotorIO fromMode(String name, AzimuthMotorHardwareConfig config,
			Supplier<AzimuthMotorIO> realFactory, Supplier<AzimuthMotorIO> simFactory) {
	...
	/**
	 * Creates mode-appropriate azimuth motor IO using TalonFX for real hardware and
	 * a custom sim source.
	 */
	public static AzimuthMotorIO fromTalonFX(String name, AzimuthMotorHardwareConfig config,
