...
public class AzimuthMotorIOSimMaple implements AzimuthMotorIO {
  ...
  public AzimuthMotorIOSimMaple(
      SwerveModuleSimulation moduleSimulation, String name, AzimuthMotorHardwareConfig config) {
    this.moduleSimulation = moduleSimulation;
    this.steerController =
        moduleSimulation
  ...
  public void updateInputs(AzimuthMotorIOInputs inputs) {
    ...
    for (int i = 0; i < config.canIds().length; i++) {
      ...
      motorVelocities[i] =
          moduleSimulation.getSteerRelativeEncoderVelocity().in(RotationsPerSecond);
