 ...
 *
 * <p>
 * Provides the SIM implementation of {@link ModuleIO} by bundling split
 * drive/azimuth IO endpoints. If/when MapleSim-specific drive/steer IO
 * implementations are added, wire them in here so both endpoints share the same
...
public class ModuleIOSim implements ModuleIO {
	...
	/**
	 * Creates a module simulation bundle from the current split sim IO
	 * implementations.
	 *
	 * @param driveName
	 *            drive motor logging name
	 * @param driveConfig
	 *            drive motor hardware/sim config
	 * @param azimuthName
	 *            azimuth motor logging name
	 * @param azimuthConfig
	 *            azimuth motor hardware/sim config
	 */
	public ModuleIOSim(String driveName, DriveMotorHardwareConfig driveConfig, String azimuthName,
	 ...
	 *
	 * <p>
	 * This overload is the integration point for Maple-backed drive/azimuth IO
	 * implementations. For now, it falls back to the standard split sim IO path
	 * until Maple-specific IO classes are introduced.
	 */
	public ModuleIOSim(SwerveModuleSimulation moduleSimulation, String driveName, DriveMotorHardwareConfig driveConfig,
			String azimuthName, AzimuthMotorHardwareConfig azimuthConfig) {
	...
	/**
	 * Creates a module simulation bundle using derived drive/steer names from a
	 * module name.
	 */
	public static ModuleIOSim forModule(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig) {
		return new ModuleIOSim(driveName(moduleName), driveConfig, azimuthName(moduleName), azimuthConfig);
	}
