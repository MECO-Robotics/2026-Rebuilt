...
/**
 * Hardware abstraction bundle for one swerve module (drive + azimuth IO
 * endpoints).
 */
public interface ModuleIO {
	...
	/**
	 * Creates a module IO supplier from independently selected drive and azimuth
	 * factories.
	 */
	public static Supplier<ModuleIO> factory(Supplier<DriveMotorIO> driveFactory,
			Supplier<AzimuthMotorIO> azimuthFactory) {
		return () -> new ModuleIO() {
	...
	public static Supplier<ModuleIO> talonFXFactory(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig) {
		return factory(DriveMotorIO.talonFXFactory(driveName(moduleName), driveConfig),
				AzimuthMotorIO.talonFXFactory(azimuthName(moduleName), azimuthConfig));
	}

	/** Creates a default simulation module IO supplier. */
	public static Supplier<ModuleIO> simFactory(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig) {
	...
	public static Supplier<ModuleIO> replayFactory(String moduleName) {
		return factory(DriveMotorIO.replayFactory(moduleName + "Drive"),
	 ...
	 */
	public static ModuleIO fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig, Supplier<ModuleIO> realFactory) {
		return fromMode(moduleName, driveConfig, azimuthConfig, realFactory,
	 ...
	 *
	 * <p>
	 * Allows callers to inject shared module simulation endpoints.
	 */
	public static ModuleIO fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
	...
	public static ModuleIO fromTalonFX(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig) {
		return fromMode(moduleName, driveConfig, azimuthConfig, talonFXFactory(moduleName, driveConfig, azimuthConfig));
	}

	/**
	 * Creates mode-appropriate module IO using TalonFX for real hardware and custom
	 * sim IO.
	 */
	public static ModuleIO fromTalonFX(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig, Supplier<ModuleIO> simFactory) {
		return fromMode(moduleName, driveConfig, azimuthConfig, talonFXFactory(moduleName, driveConfig, azimuthConfig),
				simFactory);
	}
}
