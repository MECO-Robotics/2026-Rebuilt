...
public class Module {
	 ...
	 *
	 * @param driveMotorIO
	 *            drive motor IO implementation
	 * @param azimuthMotorIO
	 *            azimuth motor IO implementation
	 */
	...
	public static Module fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig, Supplier<ModuleIO> realFactory) {
		return new Module(ModuleIO.fromMode(moduleName, driveConfig, azimuthConfig, realFactory));
	}

	/**
	 * Creates a mode-appropriate module from independently selected drive/azimuth
	 * real factory builders. Names and configs are provided once and reused for
	 * both factories.
	 */
	public static Module fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
		...
		return fromMode(moduleName, driveConfig, azimuthConfig,
				ModuleIO.factory(driveFactoryBuilder.apply(moduleName + "Drive", driveConfig),
						azimuthFactoryBuilder.apply(moduleName + "Steer", azimuthConfig)));
	}

	/**
	 * Creates a mode-appropriate module from independently selected drive/azimuth
	 * real factory builders and a custom sim factory.
	 */
	public static Module fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig,
			BiFunction<String, DriveMotorHardwareConfig, Supplier<DriveMotorIO>> driveFactoryBuilder,
	...
	/**
	 * Creates a mode-appropriate module from independently selected drive/azimuth
	 * real factory builders and optional Maple module simulation state.
	 */
	public static Module fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig,
			BiFunction<String, DriveMotorHardwareConfig, Supplier<DriveMotorIO>> driveFactoryBuilder,
		...
		Supplier<ModuleIO> simFactory = moduleSimulation != null
				? () -> new ModuleIOSim(moduleSimulation, moduleName + "Drive", driveConfig, moduleName + "Steer",
						azimuthConfig)
				: ModuleIO.simFactory(moduleName, driveConfig, azimuthConfig);
		return fromMode(moduleName, driveConfig, azimuthConfig, driveFactoryBuilder, azimuthFactoryBuilder, simFactory);
	}

	/**
	 * Creates a mode-appropriate module from real-hardware and custom sim module
	 * factories.
	 */
	public static Module fromMode(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig, Supplier<ModuleIO> realFactory, Supplier<ModuleIO> simFactory) {
		return new Module(ModuleIO.fromMode(moduleName, driveConfig, azimuthConfig, realFactory, simFactory));
	}

	/**
	 * Creates a TalonFX-based module with default sim/replay behavior and derived
	 * drive/steer names.
	 */
	public static Module fromTalonFX(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig) {
		return new Module(ModuleIO.fromTalonFX(moduleName, driveConfig, azimuthConfig));
	}

	/**
	 * Creates a TalonFX-based module with custom simulation endpoints and derived
	 * drive/steer names.
	 */
	public static Module fromTalonFX(String moduleName, DriveMotorHardwareConfig driveConfig,
			AzimuthMotorHardwareConfig azimuthConfig, ModuleIOSim simBundle) {
	...
	public void periodic() {
		...
		for (int i = 0; i < sampleCount; i++) {
			double positionMeters = driveInputs.odometryDrivePositionsRad[i] * DriveConstants.driveWheelRadiusMeters;
	...
	public double getVelocityMetersPerSec() {
		return Units.rotationsToRadians(driveInputs.velocityRotationsPerSecond) * DriveConstants.driveWheelRadiusMeters;
	}
