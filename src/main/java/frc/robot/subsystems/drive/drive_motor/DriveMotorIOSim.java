...
public class DriveMotorIOSim implements DriveMotorIO {
	...
	public DriveMotorIOSim(String name, DriveMotorHardwareConfig config) {
		...
		sim = new DCMotorSim(LinearSystemId.createDCMotorSystem(gearBox, 0.01, config.gearRatio()), gearBox);
	...
	public void updateInputs(DriveMotorIOInputs inputs) {
		...
		if (driveClosedLoop) {
			driveAppliedVolts = controller.calculate(sim.getAngularVelocity().in(RotationsPerSecond), velocitySetpoint)
					+ ffVolts;
		} else {
			controller.reset();
		}
		...
		inputs.odometryDrivePositionsRad = new double[]{Units.rotationsToRadians(inputs.positionRotations)};
	}
