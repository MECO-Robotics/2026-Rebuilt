...
public class VisionIOPhotonVisionTrig implements VisionIO {
	 ...
	 * @param gyroRotation
	 *            Supplier for robot heading used in trig pose solve.
	 */
	public VisionIOPhotonVisionTrig(String name, Transform3d robotToCamera, Supplier<Rotation2d> gyroRotation) {
	...
	public void updateInputs(VisionIOInputs inputs) {
		...
		for (var result : camera.getAllUnreadResults()) {
			...
			if (result.hasTargets()) {
				...
				Pose2d tagPose2d = VisionConstants.aprilTagLayout.getTagPose(result.getBestTarget().fiducialId).get()
						.toPose2d();

				Translation2d fieldToCamTranslation = new Pose2d(tagPose2d.getTranslation(),
						camToTagRotation.plus(Rotation2d.kPi))
								.transformBy(new Transform2d(distance2d, 0.0, Rotation2d.kZero)).getTranslation();

				Pose2d robotPose2d = new Pose2d(fieldToCamTranslation,
						gyro.plus(robotToCamera.getRotation().toRotation2d())).transformBy(
								new Transform2d(new Pose2d(robotToCamera.getTranslation().toTranslation2d(),
										robotToCamera.getRotation().toRotation2d()), Pose2d.kZero));
